  ...
  <div class="container">
    ...
    <div class="w main">
      <div class="query">
        ...
        <div v-show="query.categoryId" class="cell">
...
export default {
  ...
  methods: {
    pageLoad({isDomestic = 1, isNew = 0, cityId = '', country = '', categoryId}) {
      this.query = {
        ...this.query,
        isDomestic,
        isNew,
    ...
    // 获取全部城市
    getAllCityList() {
      this.$api.exhibition.getCity().then((res = []) => {
        this.cityList = [
          {
            abbreviation: '全部',
            cityId: '',
    ...
    getCategoryRegion() {
      this.$api.exhibition.getCategoryRegion({categoryId: this.query.categoryId}).then((res) => {
        if (!res || Array.isEmpty(res)) return;
        this.areaList = res;
        const filterItem = res.filter((item) => item.isDomestic == this.query.isDomestic);
        if (!Array.isEmpty(filterItem)) {
          this.areaActive = filterItem[0];
        }
      });
    },
    // 获取年份

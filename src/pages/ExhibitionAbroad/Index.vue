<template>
  <div class="container">
    ...
    <div class="main">
      <div class="banner">
        <div class="name">国际展会行业分类目录</div>
        <div class="en-name">INDUSTRY CLASSIFICATION OF INTERNATIONAL EXHIBITIONS</div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 获取全部分类
    getAllCategory() {
      this.$api.exhibition.getAllCategory({isDomestic: 0}).then((res) => {
        this.list = res || [];
      });
    },
    // 跳转展会列表
    onTapCate(categoryId) {
      this.navigateTo(`/exhibition?isDomestic=0&categoryId=${categoryId}`);
    },
  },
};
</script>
